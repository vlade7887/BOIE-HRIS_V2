...
@section('title', 'Create Company') @section('page_title', 'Create Company')
@section('breadcrumb')<li class="breadcrumb-item"><a href="{{ route('dashboard') }}">Dashboard</a></li><li class="breadcrumb-item"><a href="{{ route('companies.index') }}">Company</a></li><li class="breadcrumb-item active">Create</li>@endsection
@section('content')<div class="container-fluid"><div class="card"><div class="card-header"><h3 class="card-title">Company Details</h3></div><div class="card-body">
@include('partials.master-data.validation-summary')<form action="{{ route('companies.store') }}" method="POST">@csrf<div class="row g-3">
...
@include('partials.master-data.status-field', ['value' => $company->is_active ?? true])
...
</div>@include('partials.master-data.form-actions', ['submitLabel' => 'Save Company', 'cancelUrl' => route('companies.index')])</form></div></div></div>@endsection
